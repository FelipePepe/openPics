...
const WAN_I2V_MODEL = 'wan2.2_i2v_high_noise_14B_fp8_scaled.safetensors'
...
// uploadedFilename must already be in ComfyUI's /input folder (via /upload/image)
export function buildWanI2VWorkflow(prompt: string, seed: number, uploadedFilename: string) {
  return {
    ...
    // end_image key must be completely omitted (not undefined) — ComfyUI errors if present
    '10': {
